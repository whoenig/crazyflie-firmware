...
void imu6Init(void)
{
 ...
 isBaroPresent = false;
  ...
  cosPitch = cos(0); //cos(configblockGetCalibPitch() * M_PI/180);
  sinPitch = sin(0); //sin(configblockGetCalibPitch() * M_PI/180);
  cosRoll = cos(0);//cos(configblockGetCalibRoll() * M_PI/180);
  sinRoll = sin(0);//sin(configblockGetCalibRoll() * M_PI/180);
